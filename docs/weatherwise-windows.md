# Your First Project: A Claude-Powered Weather Assistant in TypeScript
## Windows edition
...
3. Which of those ideas belong to the language you happened to use, and which are real. Part 13 rebuilds the program in Python to make that line visible.

**Time:** Part 0 takes about 40 minutes. The rest is a few evenings. Don't rush; understanding beats finishing.

**Verified against:** `@anthropic-ai/sdk` v0.116.0, Zod 4.4.3, Node 22, Windows 11. Every code example typechecks under `strict: true`.
...
**Synchronous** means "one thing at a time, in order." Line 2 does not start until line 1 is completely finished. It's how you'd naturally assume code works, and it's how the Python version in Part 13 works.
...
# Part 13 — The same program in Python
...
## 13.2 The weather client

Compare this to `src/weather.ts` side by side in Cursor. Same shape, different spelling.
...
Read that against `src/agent.ts`. The structure is line-for-line the same:
...
> **Good Claude Code task:** `Port src/bench.ts to Python as pyweather/bench.py, keeping the same output format.` You understand both sides now, so you can verify the result properly — which is exactly the kind of delegation Part 1 described.
...
5. **Put a web UI on it.** A Next.js app with an API route. Keep every API call server-side — neither key may ever reach the browser.
